    ...
    def extract(self, model, loader):
        ...
        with torch.no_grad():
            for batch_x, batch_y, batch_mask in loader:
                B = batch_x.shape[0]
                valid_mask = ~batch_mask.to(self.device)
                ...
                if batch_x.ndim == 3:
                    ...
                    T = batch_x.shape[3]
                    valid_mask_expanded = valid_mask.view(B, 1, 1, T, 1, 1).expand_as(
                        batch_x
                    )
                    x_masked = batch_x.to(self.device) * valid_mask_expanded.float()
                    valid_lengths = (
                        valid_mask.sum(dim=1).clamp_min(1.0).view(B, 1, 1, 1, 1)
                    )
                    mean_features = x_masked.sum(dim=3) / valid_lengths
                    mean_features = mean_features.view(B, -1)
                else:
                    raise ValueError(
                        f"FeatureExtractor unsupported batch_x ndim: {batch_x.ndim}"
                    )
