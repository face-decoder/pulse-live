...
class AnxietyStreamProcessor:
    def __init__(
        ...
        self.all_magnitudes: list[
            float
        ] = []  # ponytail: global history for full-video graphs
    ...
    async def _process_frame(
        ...
        if len(self._magnitudes_buf) >= MIN_FRAMES and not self._inference_in_progress:
            ...
            if result is not None:
                ...
                if current_time - self._last_saved_time >= WINDOW_SECONDS:
                    ...
                    local_path = os.path.join(
                        session_dir, f"detection_{detection_id}.json"
                    )
    ...
    def _on_track(track: MediaStreamTrack) -> None:
        if track.kind == "video":
            ...
            local_track = AnxietyVideoTrack(
                track, state.result_queue, session_id=session_id
            )
