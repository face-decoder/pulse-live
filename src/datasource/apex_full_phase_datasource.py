...
class ApexFullPhaseDataSource(BaseDataSource):
    """Full phase extraction dengan temporal normalization."""

    def __init__(self, target_length: int = 32, **kwargs):
        """Inisialisasi full phase datasource.

        Args:
            target_length: jumlah frame target setelah resampling
        """
        super().__init__(strategy_name=f"full_phase_len{target_length}", **kwargs)
        self.target_length = target_length

    def __getitem__(self, index) -> Tuple[torch.Tensor, torch.Tensor, int]:
        """Mengambil sample berdasarkan full phase onset-to-offset.
        ...
        if not apex_indices or not phases:
            ...
            flow_segments = ApexFrameExtractor.extract_full_phase(
                self.spotter.flows, 0, total_frames - 1, self.target_length
            )
            frame_segments = ApexFrameExtractor.extract_full_phase(
                self.spotter.frames, 0, total_frames - 1, self.target_length
            )
        else:
            ...
            flow_segments = ApexFrameExtractor.extract_full_phase(
                self.spotter.flows, onset, offset, self.target_length
            )
            frame_segments = ApexFrameExtractor.extract_full_phase(
                self.spotter.frames, onset, offset, self.target_length
            )
