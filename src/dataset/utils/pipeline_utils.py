...
class HybridNormalizer:
    ...
    def transform(self, samples: list) -> list:
        ...
        for s in samples:
            ...
            if sig.ndim == 1:
                sig = sig[None, :]
            sig[:, self.ZSCORE_CHANNELS] = (
                sig[:, self.ZSCORE_CHANNELS] - self.mu_[:, self.ZSCORE_CHANNELS]
            ) / self.std_[:, self.ZSCORE_CHANNELS]
            denom = (
                self.xmax_[:, self.MINMAX_CHANNELS]
                - self.xmin_[:, self.MINMAX_CHANNELS]
            ) + 1e-8
            sig[:, self.MINMAX_CHANNELS] = (
                sig[:, self.MINMAX_CHANNELS] - self.xmin_[:, self.MINMAX_CHANNELS]
            ) / denom
            s["signal"] = sig
        return samples
...
def stratified_group_split(
    metadata_df, n_splits: int = 5, test_size: float = 0.2, random_state: int = 42
):
    ...
    sgkf = StratifiedGroupKFold(
        n_splits=n_splits, shuffle=True, test_size=test_size, random_state=random_state
    )
    ...
    test_df = metadata_df[
        ~metadata_df["subject_id"].isin(train_subjects | val_subjects)
    ].copy()
